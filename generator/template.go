	...
	for _, op := range operations {
		if specJSON := planSpecs[op.Name]; specJSON != "" {
			if respType := respTypeByName[op.ResponseStructName]; respType != nil {
				if g.genEntityFetch(op.Name, respType, templates.ToGo(op.ResponseStructName), specJSON) != "" {
...
func genUnmarshalJSONMethod(structName string, t types.Type) string {
	...
	for _, f := range frags {
		...
		b.WriteString("if _err := json.Unmarshal(data, &t." + f.goName + "); _err != nil { return _err }\n")
	}
	b.WriteString("}\n}\nreturn nil\n}\n")
	return b.String()
}
